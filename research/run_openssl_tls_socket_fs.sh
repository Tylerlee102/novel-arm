...
COMMON=(
  ...
  --native-self-roi
  --native-only
  --native-binary research/bin/aarch64_openssl_tls_socket_workload
  --native-arg=--sessions
  --native-arg="${SESSIONS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_ossltlssocket_${TAG}_${label}"
  echo "COPPER_OPENSSL_TLS_SOCKET_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_OPENSSL_TLS_SOCKET_APP_DONE ${label}"
}
