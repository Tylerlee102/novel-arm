...
& $Python research\build_copper_workload_clpd_replay.py | Out-Host
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$CountsPath = "research\results\copper_clpd_workload_replay_counts_20260619.json"
if (!(Test-Path $CountsPath)) {
    Write-Error "Replay count file was not generated."
    exit 1
}
...
& "$VivadoBin\xvlog.bat" -sv `
    ...
    -log research\results\copper_clpd_sram_workload_activity_xvlog.log
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& "$VivadoBin\xelab.bat" --debug wave `
    copper_clpd_sram_workload_activity_tb `
    -s copper_clpd_sram_workload_activity_sim `
    --log research\results\copper_clpd_sram_workload_activity_xelab.log
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& "$VivadoBin\xsim.bat" copper_clpd_sram_workload_activity_sim `
