...
PRESSURE_FIELDS = [
    ("membus_pkt_size_total_delta_vs_none_pct", 0.35, "bus bytes"),
    ("dram_read_reqs_delta_vs_none_pct", 0.30, "DRAM reads"),
    ("l2_replacements_delta_vs_none_pct", 0.20, "L2 replacements"),
    ("l1d_replacements_delta_vs_none_pct", 0.15, "L1D replacements"),
]
...
def pressure_score(row: dict[str, str]) -> float:
    return sum(ff(row, field) * weight for field, weight, _ in PRESSURE_FIELDS)
...
def build() -> None:
        ...
        score_rows.append(
            {
                ...
                "pressure_score_vs_none_pct": f"{pressure_score(row):.3f}",
    ...
    for row in score_rows:
        lines.append(
            ...
            f"{row['pressure_score_vs_none_pct']}% | {row['bus_delta_vs_none_pct']}% | "
