#!/usr/bin/env python3
"""Summarize libxml2 XML full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_libxml2_app"
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"LIBXML2_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_libxml2_{tag}_{policy}"
        ...
        row = {
            ...
            "dumped_bytes": info.get("dumped_bytes", ""),
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"libxml2_{tag}_summary.csv"
    ...
    lines.extend(
        [
            ...
            "Workload shape:",
            "",
            f"- XML records: {by['none'].get('records', '')}; rounds: {by['none'].get('rounds', '')}; parse/dump scans per round: {by['none'].get('scan_depth', '')}.",
            f"- XML bytes generated: {by['none'].get('xml_bytes', '')}; serialized bytes observed: {by['none'].get('dumped_bytes', '')}; address-shaped XML words: {by['none'].get('pointer_like_words', '')}.",
            "",
            "Interpretation:",
            "",
    ...
    md_path = OUT / f"LIBXML2_{tag.upper()}_FS_SUMMARY.md"
...
def main() -> None:
    ...
    parser.add_argument("--tag", default="xml_smoke")
