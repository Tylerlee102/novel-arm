...
module copper_stream_gate_tb;
    ...
    initial begin
        ...
        issue_dmp(8'h10, 2'h0, 4'h1, 4'h1, 1'b1, 1'b1);
        check("untrained stream blocks", 1'b0, 1'b1);

        commit_source(8'h10, 2'h0, 4'h1);
        commit_source(8'h11, 2'h0, 4'h1);
        commit_source(8'h12, 2'h0, 4'h1);
        ...
        if (!stream_trained) begin
            $error("stream should be trained after threshold commits");
        end

        issue_dmp(8'h10, 2'h0, 4'h1, 4'h1, 1'b1, 1'b1);
        check("trained clean source allows", 1'b1, 1'b0);
        ...
        commit_source(8'h10, 2'h0, 4'h1);
        issue_dmp(8'h10, 2'h0, 4'h1, 4'h1, 1'b1, 1'b1);
        check("commit clears dirty source", 1'b1, 1'b0);

        issue_dmp(8'h10, 2'h0, 4'h1, 4'h2, 1'b1, 1'b1);
        check("domain mismatch blocks", 1'b0, 1'b1);

        issue_dmp(8'h10, 2'h0, 4'h1, 4'h1, 1'b0, 1'b1);
        check("translation failure blocks", 1'b0, 1'b1);

        issue_dmp(8'h10, 2'h0, 4'h1, 4'h1, 1'b1, 1'b0);
        ...
        if (!dirty_overflow) begin
            $error("dirty table overflow should be sticky");
        end

        issue_dmp(8'h12, 2'h0, 4'h1, 4'h1, 1'b1, 1'b1);
        check("dirty overflow fails safe", 1'b0, 1'b1);

        $display("COPPER stream gate directed tests completed");
        $finish;
    end
