#!/usr/bin/env python3
"""Summarize TLS/session-service full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_tlssvc_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"TLSSVC_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_tlssvc_{tag}_{policy}"
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"tlssvc_{tag}_summary.csv"
    ...
    poison_values = {row.get("poison", "") for row in rows}
    payload_description = (
        "Ticket/mask fields contain pointer-shaped 64-bit words that are loaded by the authentication loop but never used as architectural addresses."
        if poison_values != {"0"}
        else "Ticket/mask fields are constrained below the DMP candidate address range and are loaded by the authentication loop but never used as architectural addresses."
    )
    ...
    lines = [
        ...
        f"constant-time-ish record-authentication arithmetic. {payload_description}",
        "",
        f"Input tag: `{tag}`.",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "status=PASS" if len(checksums) == 1 and "" not in checksums else "status=CHECKSUM_MISMATCH",
            "",
        ]
    )
    md_path = OUT / f"TLSSVC_{tag.upper()}_FS_SUMMARY.md"
