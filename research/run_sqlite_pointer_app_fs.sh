...
RANGES=${RANGES:-128}
UPDATES=${UPDATES:-512}
PAYLOAD_ROWS=${PAYLOAD_ROWS:-1024}
SEED=${SEED:-0}
POLICY_LIST=${POLICY_LIST:-"none stride naive copper_clpd64k_peb dcpt spp ampm"}
EXTRA_NATIVE_ARGS=()
...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_sqlite_pointer_workload
  --native-arg=--rows
  --native-arg="${ROWS}"
  --native-arg=--lookups
  --native-arg="${LOOKUPS}"
  ...
  --native-arg=--updates
  --native-arg="${UPDATES}"
  --native-arg=--payload-rows
  --native-arg="${PAYLOAD_ROWS}"
  --native-arg=--seed
  --native-arg="${SEED}"
  "${EXTRA_NATIVE_ARGS[@]}"
)
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_sqlite_${TAG}_${label}"
  echo "COPPER_SQLITE_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_SQLITE_APP_DONE ${label}"
}
