#!/usr/bin/env python3
"""Summarize Lua table/runtime full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_lua_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"LUA_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_lua_{tag}_{policy}"
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"lua_{tag}_summary.csv"
    ...
    out = OUT / f"LUA_{tag.upper()}_FS_SUMMARY.md"
