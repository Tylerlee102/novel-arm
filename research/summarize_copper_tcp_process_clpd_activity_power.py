#!/usr/bin/env python3
"""Summarize TCP process-server CLPD activity power evidence."""
...
OUT = RESULTS / "COPPER_TCP_PROCESS_CLPD_ACTIVITY_POWER_20260620.md"
...
def main() -> None:
    counts_path = RESULTS / "copper_clpd_tcp_process_replay_counts_20260620.json"
    counts = json.loads(counts_path.read_text(encoding="utf-8"))
    xsim = read(RESULTS / "copper_clpd_sram_tcp_process_activity_xsim.log")
    power = read(RESULTS / "copper_clpd_sram_tcp_process_activity_saif_power.rpt")
    timing = read(RESULTS / "copper_clpd_sram_tcp_process_activity_timing.rpt")
    manifest = read(RESULTS / "copper_clpd_sram_tcp_process_activity_saif_power_manifest_20260620.csv")
    ...
    saif = RESULTS / "copper_clpd_sram_tcp_process_activity.saif"
    unmatched = RESULTS / "copper_clpd_sram_tcp_process_activity_saif_unmatched.txt"
    status = "PASS" if "READ_SAIF_OK" in manifest and "errors=0" in replay_line else "CHECK"
    ...
    for source_csv in counts["source_csvs"]:
        lines.append(f"- Source CSV: `{source_csv}`")
    lines.extend(
        [
            f"- Source policy: `{counts['policy']}`",
            f"- Policy choice: {counts['why_policy']}",
            f"- Seed points: {counts['seed_points']}",
            ...
            f"- Useful prefetches in source rows: {raw['pf_useful']:,}",
            f"- Process TCP pairs in selected rows: {raw['process_pairs']:,}",
            f"- Child failures in selected rows: {raw['child_failures']:,}",
            f"- Translation faults in selected rows: {raw['translation_faults']:,}",
            "",
            ...
            "|---|---:|---:|---:|---:|---:|---|",
            f"| TCP process-server SAIF | {matched} | {total_power} | {dynamic_power} | {static_power} | {confidence} | WNS {wns} ns |",
            "",
            "## Component Breakdown",
            "",
