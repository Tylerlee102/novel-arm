...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_tls_session_workload
  --native-arg=--sessions
  --native-arg="${SESSIONS}"
  --native-arg=--requests
  --native-arg="${REQUESTS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_tlssvc_${TAG}_${label}"
  echo "COPPER_TLSSVC_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_TLSSVC_APP_DONE ${label}"
}
