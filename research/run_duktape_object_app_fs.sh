...
TAG=${TAG:-app_small}
ROWS=${ROWS:-512}
LOOKUPS=${LOOKUPS:-1200}
UPDATES=${UPDATES:-512}
TRAVERSALS=${TRAVERSALS:-1200}
SEED=${SEED:-0}
...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_duktape_object_workload
  --native-arg=--rows
  --native-arg="${ROWS}"
  --native-arg=--lookups
  --native-arg="${LOOKUPS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_duktape_${TAG}_${label}"
  echo "COPPER_DUKTAPE_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_DUKTAPE_APP_DONE ${label}"
}
