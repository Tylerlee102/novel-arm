#!/usr/bin/env python3
"""Summarize two-seed JSON+SQLite service-composition stability."""
...
OUT = RESULTS / "JSONSQLITE_MEDIUM_SEED_STABILITY_20260619.md"

POINTS = [
    ("app_medium", APP_DIR / "jsonsqlite_app_medium_summary.csv"),
    ("medium_seed1", APP_DIR / "jsonsqlite_medium_seed1_summary.csv"),
]
...
def main() -> None:
    ...
    lines.extend(
        [
            ...
            "Aggregate:",
            "",
            f"- Points: {len(summary)} medium JSON+SQLite seeds.",
            f"- COPPER CTLW reduction is at least {min_copper:.1f}% across the two seeds.",
            f"- SPP+COPPER slack CTLW reduction is at least {min_slack:.1f}% across the two seeds.",
