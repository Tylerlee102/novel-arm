...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_pcre2_regex_workload
  --native-arg=--records
  --native-arg="${RECORDS}"
  --native-arg=--lookups
  --native-arg="${LOOKUPS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_pcre2_${TAG}_${label}"
  echo "COPPER_PCRE2_REGEX_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_PCRE2_REGEX_APP_DONE ${label}"
}
