#!/usr/bin/env python3
"""Summarize OpenSSL libssl memory-BIO TLS full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_ossltlsbio_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"OPENSSL_TLS_BIO_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"ossltlsbio_{tag}_summary.csv"
    ...
    md_path = OUT / f"OSSLTLSBIO_{tag.upper()}_FS_SUMMARY.md"
