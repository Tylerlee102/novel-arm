...
TAG=${TAG:-app_small}
ROWS=${ROWS:-1024}
LOOKUPS=${LOOKUPS:-3000}
UPDATES=${UPDATES:-1024}
TRAVERSALS=${TRAVERSALS:-3000}
SEED=${SEED:-0}
POLICY_LIST=${POLICY_LIST:-"none naive copper_clpd64k_peb spp"}
EXTRA_NATIVE_ARGS=()
...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_lua_table_workload
  --native-arg=--rows
  --native-arg="${ROWS}"
  --native-arg=--lookups
  --native-arg="${LOOKUPS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_lua_${TAG}_${label}"
  echo "COPPER_LUA_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_LUA_APP_DONE ${label}"
}
