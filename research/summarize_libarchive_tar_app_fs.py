#!/usr/bin/env python3
"""Summarize libarchive TAR full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_libarchive_app"
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"LIBARCHIVE_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
        info["checksum"] = result.group("checksum")
        for key, value in re.findall(r"(\w+)=([0-9a-fA-Fx]+)", result.group("body")):
            info[key] = value
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_libarchive_{tag}_{policy}"
        ...
        row = {
            ...
            "seed": info.get("seed", ""),
            "archive_bytes": info.get("archive_bytes", ""),
            "payload_bytes": info.get("payload_bytes", ""),
            "pointer_like_words": info.get("pointer_like_words", ""),
        }
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"libarchive_{tag}_summary.csv"
    ...
    none_ticks = int(by["none"]["roi_ticks"])
    spp_delta = ((spp_ticks / none_ticks) - 1.0) * 100.0 if none_ticks else 0.0
    slack_delta = ((slack_ticks / none_ticks) - 1.0) * 100.0 if none_ticks else 0.0
    ...
    lines.extend(
        [
            ...
            "Workload shape:",
            "",
            f"- TAR entries: {by['none'].get('entries', '')}; rounds: {by['none'].get('rounds', '')}; archive scans per round: {by['none'].get('scans', '')}.",
            f"- Archive bytes generated: {by['none'].get('archive_bytes', '')}; payload bytes read: {by['none'].get('payload_bytes', '')}; address-shaped archive words: {by['none'].get('pointer_like_words', '')}.",
            "",
            "Interpretation:",
            "",
            ...
            f"- SPP tick delta: {spp_delta:.3f}%; SPP+COPPER slack tick delta: {slack_delta:.3f}%; slack gap: {slack_delta - spp_delta:.3f} percentage points.",
            f"- COPPER CLPD-64K+PEB translation faults: {copper_faults}; SPP+COPPER slack translation faults: {slack_faults}.",
            ...
            "",
            f"status={'PASS' if len(checksums) == 1 and rcs == {'0'} and copper_faults == 0 and slack_faults == 0 else 'CHECK'}",
            "",
        ]
    )
    md_path = OUT / f"LIBARCHIVE_{tag.upper()}_FS_SUMMARY.md"
...
def main() -> None:
    ...
    parser.add_argument("--tag", default="tar_tiny")
