...
> Legacy evidence record: this file is not the submission claim authority. Use `research/COPPER_CLAIM_LEDGER.md`, `research/paper/main.tex`, and `research/COPPER_SYNCHRONIZED_HARDWARE_EVIDENCE_REPORT.md` for current allowed claims.

Status: full paper draft, 8-page target when formatted with standard architecture-paper tables and figures.

Restructure note: `research/COPPER_SUBMISSION_RESTRUCTURED.md` is now the submission-facing paper narrative. This file remains the evidence-heavy full record with dated addenda, artifact inventory, and source-backed claim detail.

Audit refresh: The energy/pollution scorecard's 18.8% lower proxy pollution score is the named base weighting, not a calibrated platform weight. Across the weight-sensitivity sweep, standalone COPPER's lower proxy-pollution result ranges from 18.1% to 20.6% versus naive DMP, with COPPER lower-or-equal on 20/22 points under all checked weightings. The synthetic trace report now includes cross-domain secret-rate sensitivity from 0.00 to 1.00, and the CS-SARI sensitivity artifact includes a sampled source/target-correlation addendum over `kron_g13`/`kron_g14` with zero scoped unsafe modeled issues and 83.57%-84.04% hold reduction.
...
- `research/results/COPPER_METADATA_TOGGLE_BOUND_20260619.md`: metadata-toggle energy bound over the measured 22-row metadata-authority event mix.
