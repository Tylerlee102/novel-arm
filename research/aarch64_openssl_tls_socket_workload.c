...
{
    fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_STAGE %s\n", name);
    fflush(stderr);
}
...
print_ssl_errors(const char *label)
{
    ...
    while ((err = ERR_get_error()) != 0) {
        ...
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR %s: %s\n", label, buf);
        saw = 1;
    }
    if (!saw) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR %s\n", label);
    }
}
...
tls_engine_init(tls_engine_t *engine, uint32_t seed)
{
    ...
    fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_STAGE rand_status=%d\n",
            RAND_status());
    fflush(stderr);
    stage("ctx_new");
...
tls_handshake_pair(SSL *client, SSL *server)
{
    ...
    if (!client_done || !server_done) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR handshake timeout\n");
        return 0;
    }
    return 1;
}
...
ssl_read_retry(SSL *ssl, unsigned char *buf, int len)
{
    ...
    fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR SSL_read timeout\n");
    return -1;
}
...
tls_exchange(tls_engine_t *engine, session_t *s, uint32_t records)
{
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, sv) != 0) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR socketpair failed\n");
        SSL_free(client);
        SSL_free(server);
        return 0;
    }
    if (!make_socket_nonblocking(sv[0]) || !make_socket_nonblocking(sv[1])) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR fcntl nonblock failed\n");
        SSL_free(client);
        SSL_free(server);
    ...
    for (uint32_t r = 0; r < records; ++r) {
        ...
        if (got != (int)sizeof(msg)) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR server read got=%d\n",
                    got);
            SSL_free(client);
            SSL_free(server);
            close_socket_pair(sv);
        ...
        if (back != got) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR client read got=%d\n",
                    back);
            SSL_free(client);
            SSL_free(server);
            close_socket_pair(sv);
...
service_init(service_t *svc, uint32_t sessions, uint32_t seed, int poison)
{
    ...
    if (!svc->sessions || !svc->buckets) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR allocation failed\n");
        exit(1);
    }
                ...
                int poison)
{
    ...
    if (s) {
        if (!tls_exchange(engine, s, records)) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR tls exchange failed\n");
            exit(1);
        }
        lru_touch(svc, s);
    } else {
        ...
        if (!victim) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR empty LRU\n");
            exit(1);
        }
...
validate_service(service_t *svc)
{
    ...
    for (session_t *cur = svc->lru_head; cur; cur = cur->lru_next) {
        if (cur->lru_prev != prev) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR broken lru prev\n");
            exit(1);
        }
        fold(cur->sid ^ cur->seq);
        prev = cur;
        ...
        if (lru_count > svc->sessions_count) {
            fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR lru cycle\n");
            exit(1);
        }
    }
    if (prev != svc->lru_tail || lru_count != svc->sessions_count) {
        fprintf(stderr,
                "OPENSSL_TLS_SOCKET_COPPER_ERROR lru count=%u sessions=%u\n",
...
main(int argc, char **argv)
{
    ...
    if (!is_power_of_two(sessions) || sessions < 16 || sessions > (1u << 16)) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR sessions must be a power of two in [16,65536]\n");
        return 2;
    }
    if (handshakes == 0 || handshakes > (1u << 20) || records == 0 ||
        records > 8 || rounds == 0 || rounds > 64) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR invalid handshakes, records, or rounds\n");
        return 2;
    }
    if (scan_depth == 0 || scan_depth > sessions) {
        fprintf(stderr, "OPENSSL_TLS_SOCKET_COPPER_ERROR scan-depth must be in [1,sessions]\n");
        return 2;
    }
    ...
    printf("OPENSSL_TLS_SOCKET_COPPER_RESULT sessions=%u handshakes=%u records=%u scan_depth=%u rounds=%u seed=%u poison=%u checksum=0x%016llx\n",
