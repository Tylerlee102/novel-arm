...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_expat_xml_workload
  --native-arg=--records
  --native-arg="${RECORDS}"
  --native-arg=--attrs
  --native-arg="${ATTRS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_expat_${TAG}_${label}"
  echo "COPPER_EXPAT_XML_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_EXPAT_XML_APP_DONE ${label}"
}
