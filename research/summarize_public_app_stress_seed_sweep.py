#!/usr/bin/env python3
"""Aggregate stress public-engine seed runs for SQLite, Lua, and Duktape."""
...
def main() -> None:
    ...
    for engine, summarizer in ENGINES.items():
        for seed, tag in SEEDS:
            ...
            except Exception as exc:  # report all missing runs instead of hiding first one
                missing.append(f"{engine} seed={seed} tag={tag}: {exc}")
                continue
            for row in rows:
                row["engine"] = engine
                row["seed"] = str(seed)
    ...
    if missing:
        raise RuntimeError("missing stress seed data:\n" + "\n".join(missing))
    ...
    lines = [
        ...
        *aggregate_lines,
        ...
        f"- Standalone COPPER beats unsafe naive DMP on {copper_beats_naive}/{points} stress engine-seed points.",
    ...
    out = OUT / "COPPER_PUBLIC_APP_STRESS_SEED_SWEEP_20260617.md"
