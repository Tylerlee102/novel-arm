...
def write_markdown(
    ...
    domain_lines = [
        "| Cross-domain secret rate | Naive speedup | Naive cross-domain | COPPER-LINE speedup | COPPER-LINE cross-domain |",
        "|---:|---:|---:|---:|---:|",
    ]
    for domain_rate in (0.0, 0.25, 0.50, 0.75, 1.0):
        variant = f"cross_domain_secret_rate={domain_rate:.2f}"
        subset = [row for row in rows if row["experiment"] == "domain_mix_sweep" and row["variant"] == variant]
        ...
        domain_lines.append(
            f"| {domain_rate:.2f} | {m('naive', 'speedup'):.3f}x | "
            f"{m('naive', 'cross_domain'):.1f} | "
            f"{m('copper_line', 'speedup'):.3f}x | "
            f"{m('copper_line', 'cross_domain'):.1f} |"
        )
...
## Cross-Domain Mix Sensitivity

The main synthetic trace uses `cross_domain_secret_rate=0.50`. This sweep keeps
the same generated-trace shape and varies that rate so the cross-domain count is
reported as a sensitivity result rather than an unexplained single default.
...
def main() -> None:
    ...
    for domain_rate in (0.0, 0.25, 0.50, 0.75, 1.0):
        for seed in range(2300, 2310):
            rows = make_synthetic(
                synthetic_args(seed=seed, cross_domain_secret_rate=domain_rate)
            )
            results = run_trace(rows_to_events(rows))
            add_rows(
                all_rows,
                "domain_mix_sweep",
                f"cross_domain_secret_rate={domain_rate:.2f}",
                results,
            )
