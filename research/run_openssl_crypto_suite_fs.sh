...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_openssl_crypto_suite_workload
  --native-arg=--sessions
  --native-arg="${SESSIONS}"
  --native-arg=--requests
  --native-arg="${REQUESTS}"
  --native-arg=--crypto-rounds
  --native-arg="${CRYPTO_ROUNDS}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_osslcrypto_${TAG}_${label}"
  echo "COPPER_OPENSSL_CRYPTO_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_OPENSSL_CRYPTO_APP_DONE ${label}"
}
