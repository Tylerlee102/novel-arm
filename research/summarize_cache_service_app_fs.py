#!/usr/bin/env python3
"""Summarize cache-service full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_cachesvc_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_cachesvc_{tag}_{policy}"
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"cachesvc_{tag}_summary.csv"
    ...
    payload_description = (
        "Payload fields contain pointer-shaped 64-bit words that are loaded and checksummed but never used as architectural addresses."
        if poison_values != {"0"}
        else "Payload fields are constrained below the DMP candidate address range and are loaded and checksummed but never used as architectural addresses."
    )
    ...
    lines = [
        ...
        f"evictions, and bounded hot-list scans. {payload_description}",
        "",
        f"Input tag: `{tag}`.",
        "",
    ...
    md_path = OUT / f"CACHESVC_{tag.upper()}_FS_SUMMARY.md"
...
def main() -> None:
    ...
    parser.add_argument("--tag", default="app_medium")
