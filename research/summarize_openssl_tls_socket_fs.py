#!/usr/bin/env python3
"""Summarize OpenSSL libssl socket-backed TLS full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_ossltlssocket_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"OPENSSL_TLS_SOCKET_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_ossltlssocket_{tag}_{policy}"
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"ossltlssocket_{tag}_summary.csv"
    ...
    md_path = OUT / f"OSSLTLSSOCKET_{tag.upper()}_FS_SUMMARY.md"
