...
> Legacy research-output record: this file is not the submission claim authority. Use `research/COPPER_CLAIM_LEDGER.md`, `research/paper/main.tex`, and `research/COPPER_SYNCHRONIZED_HARDWARE_EVIDENCE_REPORT.md` for current allowed claims.

Date: 2026-06-12, updated 2026-06-20

Audit refresh: The 18.8% lower proxy pollution score is the transparent base weighting, not a calibrated platform weight. Across the weight-sensitivity sweep, standalone COPPER's lower proxy-pollution result ranges from 18.1% to 20.6% versus naive DMP and is lower-or-equal on 20/22 points under all checked weightings. The synthetic-trace result now reports cross-domain secret-rate sensitivity from 0.00 to 1.00, and the CS-SARI sensitivity artifact adds a sampled source/target-correlation check over `kron_g13`/`kron_g14` with zero scoped unsafe modeled issues.
...
A metadata-toggle sensitivity bound then translates the 22-row app/service/parser/compression/TCP side-effect event mix into pJ/access accounting. Across the matching COPPER rows, the model counts 40,058 learned-proof writes, 1,407,655 CLPD source-proof reads, and 20,046 CTLW target-witness lookups charged as metadata reads. Under a deliberately high 20 pJ read, 40 pJ write, and 5 pJ compare/event scenario, estimated metadata access energy is 37.495 uJ, 0.1801% of matching COPPER DRAM operation energy and 0.002190% of total DRAM energy. This is still not SRAM compiler or silicon power, but it makes the "metadata energy probably does not dominate" claim quantitative and rerunnable.
...
| Metadata-toggle sensitivity bound | measured 22-row metadata authority event mix | high scenario: 37.495 uJ metadata, 0.1801% of matching DRAM op energy | 40,058 writes; 1,407,655 CLPD source reads; 20,046 CTLW target-witness lookups | Quantifies metadata access order of magnitude; still not SRAM compiler or silicon power. |
...
| OpenSSL TCP process-server four-point portfolio | four points, four checksums | COPPER min CTLW reduction 98.2%, 0 faults | SPP+COPPER slack min CTLW reduction 98.1%, worst gap 0.130 pp | 70 total forked TCP pairs, 0 child failures, all rows `tcp_loopback_netns_process`; still bounded local harness. |
