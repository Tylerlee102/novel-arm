...
TAG=${TAG:-speedtest1_main_size3}
SIZE=${SIZE:-3}
TESTSET=${TESTSET:-main}
REPEAT=${REPEAT:-1}
...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_sqlite_speedtest1
  --native-arg=--memdb
  --native-arg=--verify
  --native-arg=--stats
  --native-arg=--size
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_sqlite_speedtest1_${TAG}_${label}"
  echo "COPPER_SQLITE_SPEEDTEST1_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_SQLITE_SPEEDTEST1_DONE ${label}"
}
