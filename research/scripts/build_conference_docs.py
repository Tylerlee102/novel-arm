...
def gem5_raw_rerun_summary() -> str:
    ...
    return (
        f"{len(rows)} fresh local raw gem5 full-system smoke rerun rows "
...
Leaning: workshop accept / top-tier weak reject. Strengths: clear committed-provenance invariant, CI-proven RTL unit simulation, source workload build path, deterministic cycle-model rows, deterministic core-integrated rows, an independent source-backed trace/event simulator across the required workload/config matrix, imported gem5 ARM-system rows with checksum/return-code agreement ({gem5_summary}) plus imported-summary confidence intervals where repeated samples exist, and a fresh local raw gem5 smoke rerun manifest ({raw_summary}). Weaknesses: most gem5 evidence is still imported from validated summaries, and the fresh raw rerun is only a smoke pair, so it still does not prove a complete top-tier simulator campaign with fresh raw-run confidence intervals. Fatal blockers for top-tier claims: no true full-core PPA and no full-core post-route/silicon-grade power; the ASIC-Liberty evidence is scoped to the PicoRV32 core-wrapper. Required fixes: validate the final workload/config matrix in gem5 or another accepted external core simulator with a reproducible raw-run path. Claim risks: performance claims must stay per-row and evidence-level scoped. Phase 0 discrepancy check: claimed PR/push evidence matched; main-branch Actions state was not verifiable because the API returned no main runs, so main-branch status must not be cited.
