...
Evidence used by the paper and dashboard comes from generated CSVs and explicit logs. Gem5 rows are promoted only from validated PASS summaries with a no-prefetch baseline, a COPPER-family row, matching checksums, clean return codes, and positive tick counts; the current imported rows span multiple ARM-system benchmark families but are still imported summaries, not a clone-local rerun of every raw simulation. OpenROAD post-route rows are tool estimates only when OpenROAD-flow-scripts emits real route/final reports; ASIC Liberty rows are standard-cell tool estimates only when OpenSTA/OpenROAD emits a real report; Vivado report_power rows are FPGA tool estimates. None should be called measured silicon or full-core signoff power. Paper claims are controlled by `research/COPPER_CLAIM_LEDGER.md`.
...
Leaning: workshop accept / top-tier weak reject. Strengths: clear committed-provenance invariant, CI-proven RTL unit simulation, source workload build path, deterministic cycle-model rows, deterministic core-integrated rows, an independent source-backed trace/event simulator across the required workload/config matrix, and imported gem5 ARM-system rows with checksum/return-code agreement ({gem5_summary}). Weaknesses: gem5 evidence is imported from validated summaries rather than rerun clone-locally, and it still does not prove a complete top-tier simulator campaign with fresh confidence intervals. Fatal blockers for top-tier claims: no true full-core PPA and no full-core post-route/silicon-grade power; the ASIC-Liberty evidence is scoped to the PicoRV32 core-wrapper. Required fixes: validate the final workload/config matrix in gem5 or another accepted external core simulator with a reproducible raw-run path. Claim risks: performance claims must stay per-row and evidence-level scoped. Phase 0 discrepancy check: claimed PR/push evidence matched; main-branch Actions state was not verifiable because the API returned no main runs, so main-branch status must not be cited.
