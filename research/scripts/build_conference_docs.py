...
def gate_status() -> list[dict[str, str]]:
    ...
    return [
        ...
        gate("G17. Statistical stability across seeds/input sizes", "Yes", "PASS" if cycle_stats_pass() else ("PARTIAL" if (RESULTS / "statistical_summary.csv").exists() or gem5_stats_pass() else "TODO"), "research/results/seed_stability.csv; research/results/statistical_summary.csv; research/results/gem5_statistical_summary.csv", "Cycle-model stability covers seeds 1-3 and multiple input sizes; gem5 statistics summarize validated imported ARM-system rows when present", ""),
...
def build_claim_ledger() -> None:
    ...
    claims = [
        ...
        ("C15", "COPPER has validated imported gem5 ARM-system evidence across multiple benchmark families.", c15_status, "research/results/gem5_validation.csv; research/results/gem5_performance.csv; research/results/gem5_prefetch_metrics.csv; research/results/gem5_memory_traffic.csv; research/results/gem5_statistical_summary.csv; research/results/logs/gem5/gem5_import.log", "gem5_full_system", f"Allowed only for imported summary groups with a no-prefetch baseline, a COPPER-family row, matching checksums, rc=0, and positive tick counts; current scope is {gem5_evidence_summary()}. gem5_statistical_summary.csv is imported-summary statistics, not a fresh clone-local raw gem5 rerun."),
    ]
    lines = [
        "# COPPER Claim Ledger",
        "",
...
Leaning: workshop accept / top-tier weak reject. Strengths: clear committed-provenance invariant, CI-proven RTL unit simulation, source workload build path, deterministic cycle-model rows, deterministic core-integrated rows, an independent source-backed trace/event simulator across the required workload/config matrix, and imported gem5 ARM-system rows with checksum/return-code agreement ({gem5_summary}) plus imported-summary confidence intervals where repeated samples exist. Weaknesses: gem5 evidence is imported from validated summaries rather than rerun clone-locally, so it still does not prove a complete top-tier simulator campaign with fresh raw-run confidence intervals. Fatal blockers for top-tier claims: no true full-core PPA and no full-core post-route/silicon-grade power; the ASIC-Liberty evidence is scoped to the PicoRV32 core-wrapper. Required fixes: validate the final workload/config matrix in gem5 or another accepted external core simulator with a reproducible raw-run path. Claim risks: performance claims must stay per-row and evidence-level scoped. Phase 0 discrepancy check: claimed PR/push evidence matched; main-branch Actions state was not verifiable because the API returned no main runs, so main-branch status must not be cited.
...
Leaning: workshop accept if scoped, top-tier weak reject. Strengths: deterministic cycle and core-integrated rows cover seeds 1-3, multiple input sizes, and both positive/control/stress workloads; the independent simulator executes the source-built workload binary and retains regressions; imported gem5 ARM-system summaries are validated with checksum/return-code checks ({gem5_summary}) and imported-summary confidence intervals are emitted where repeated samples exist. Weaknesses: the gem5 rows are imported summaries and do not yet provide fresh raw-run confidence intervals for a final top-tier workload/config matrix. Fatal blockers for top-tier claims: no broad external simulator statistics with a reproducible raw-run path. Required fixes: add gem5 or another accepted external simulator run for the same workload/config matrix and confidence intervals from those runs. Claim risks: robust speedup must be described per benchmark/configuration, not as a suite-wide win. Phase 0 discrepancy check: row counts matched claimed deterministic-model rows.
...
| --- | --- | --- | --- |
| SERIOUS BUT CAVEATABLE | Gem5 evidence is imported from validated ARM-system summaries ({gem5_summary}) and now has imported-summary statistics where repeated samples exist; independent_sim remains source-backed trace/event validation. | gem5_performance.csv; gem5_prefetch_metrics.csv; gem5_memory_traffic.csv; gem5_statistical_summary.csv; independent_sim_performance.csv; independent_sim_prefetch_metrics.csv; independent_sim_memory_traffic.csv | Run the final full workload/config matrix in gem5 or another accepted external simulator with a reproducible raw-run path and raw-run confidence intervals before making top-tier architecture claims. |
