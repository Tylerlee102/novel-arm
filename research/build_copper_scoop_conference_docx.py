...
def build() -> None:
    ...
    paragraph(
        doc,
        "A new gem5-counter pressure scorecard separates speed from side effects. Using a transparent weighted proxy over bus bytes, DRAM reads, L2 replacements, and L1D replacements, standalone COPPER scores 1.460% versus 1.684% for naive DMP, a 13.3% lower mean proxy pollution score, while reducing CTLW misses by 91.1%. SCOOP inherits SPP's high-traffic profile, but the incremental cost is now quantified: it is within 0.5% runtime of SPP on all 12 points and adds 0.184 bus-byte-delta points and 0.396 pressure-score points over SPP on average. This is not calibrated power.",
    )
    paragraph(
        doc,
    ...
    table(
        ...
        [
            ...
            ["COPPER_ENERGY_POLLUTION_SCORECARD_20260617.md", "Traffic/pollution proxy score: COPPER 13.3% lower than naive; SCOOP incremental cost over SPP quantified."],
