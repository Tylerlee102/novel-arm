#!/usr/bin/env python3
"""Summarize yyjson full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_yyjson_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"YYJSON_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_yyjson_{tag}_{policy}"
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    csv_path = OUT / f"yyjson_{tag}_summary.csv"
    ...
    md_path = OUT / f"YYJSON_{tag.upper()}_FS_SUMMARY.md"
