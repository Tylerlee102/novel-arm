#!/usr/bin/env python3
"""Summarize zlib full-system COPPER runs."""
...
OUT = RESULTS / "gem5_arm_ubuntu_fs_zlib_app"
POLICIES = [
    "none",
    "stride",
    "naive",
...
def terminal_info(path: Path) -> dict[str, str]:
    ...
    info: dict[str, str] = {}
    result = re.search(
        r"ZLIB_COPPER_RESULT\s+(?P<body>.*?)checksum=(?P<checksum>0x[0-9a-fA-F]+)",
        text,
    )
    if result:
...
def summarize(tag: str, policies: list[str]) -> list[dict[str, str]]:
    ...
    for policy in policies:
        run_dir = RESULTS / f"gem5_arm_ubuntu_fs_zlib_{tag}_{policy}"
        ...
        row = {
            ...
            "total_compressed": info.get("total_compressed", ""),
            "zcrc": info.get("zcrc", ""),
        }
...
def write_outputs(tag: str, rows: list[dict[str, str]]) -> None:
    ...
    lines.extend(
        [
            ...
            "Workload shape:",
            "",
            f"- Input bytes: {by['none'].get('bytes', '')}; rounds: {by['none'].get('rounds', '')}; compression level: {by['none'].get('level', '')}.",
            f"- Address-shaped input words: {by['none'].get('pointer_like_words', '')}; total compressed bytes across rounds: {by['none'].get('total_compressed', '')}; zlib CRC aggregate: {by['none'].get('zcrc', '')}.",
            "",
            "Interpretation:",
            "",
    ...
    md_path = OUT / f"ZLIB_{tag.upper()}_FS_SUMMARY.md"
