...
| G6. C benchmark/workload build | Yes | PARTIAL | research/results/workload_build.csv; research/workloads/copper_workload_suite.c | Required workload suite builds from source and records one row per benchmark | Current environment has not produced PASS rows for every required workload build. |
| G7. Benchmark execution | Yes | PASS | research/results/performance.csv; research/results/cycle_performance.csv; research/results/core_integrated_performance.csv | Model, cycle_model, and core_integrated rows exist for the public benchmark suite |  |
...
| G10. Prefetch usefulness/accuracy/coverage metrics | Yes | PASS | research/results/prefetch_metrics.csv; research/results/cycle_prefetch_metrics.csv; research/results/core_integrated_prefetch_metrics.csv | Issued/useful/useless/late/queue/coverage/accuracy metrics are generated with evidence-level labels |  |
| G11. Speedup/performance metrics | Yes | PASS | research/results/performance.csv; research/results/cycle_performance.csv; research/results/core_integrated_performance.csv | Per-workload speedup versus no-prefetch is reported with evidence-level labels |  |
| G12. Memory traffic/bandwidth overhead metrics | Yes | PASS | research/results/memory_traffic.csv; research/results/cycle_memory_traffic.csv; research/results/core_integrated_memory_traffic.csv | Traffic overhead is generated from model, cycle-model, and core-integrated request counts where available |  |
...
| G15. Area/resource/timing synthesis | Yes | PASS | research/results/synthesis.csv; research/results/synthesis_overhead.csv; research/results/fullcore_synthesis.csv; research/results/fullcore_synthesis_overhead.csv | Matched unit-level and, when available, near-core-stub overhead exists from the same flow |  |
| G16. Power/energy proxy or measured estimate | Yes | PARTIAL | research/results/energy_proxy.csv; research/results/energy_summary.csv | Proxy energy rows are generated with explicit assumptions | Proxy is assumption-based and not calibrated measured power. |
