...
COMMON=(
  ...
  --native-only
  --native-binary research/bin/aarch64_json_sqlite_ingest_workload
  --native-arg=--rows
  --native-arg="${ROWS}"
  --native-arg=--queries
  --native-arg="${QUERIES}"
...
run_policy() {
  local label="$1"
  shift
  local outdir="research/results/gem5_arm_ubuntu_fs_jsonsqlite_${TAG}_${label}"
  echo "COPPER_JSONSQLITE_APP_START ${label}"
  ./external/gem5/build/ARM/gem5.fast.exe --outdir="${outdir}" "${COMMON[@]}" "$@" \
    > "${outdir}.host.out" 2> "${outdir}.host.err"
  echo "COPPER_JSONSQLITE_APP_DONE ${label}"
}
